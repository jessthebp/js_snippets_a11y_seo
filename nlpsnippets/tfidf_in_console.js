...
function calculateTFIDF(document, corpus) {
  ...
  Object.keys(wordCounts).forEach((word) => {
    const numDocumentsContainingWord = corpus.filter((doc) => doc.includes(word)).length;
    const idf = Math.log10((totalDocuments + 1) / (numDocumentsContainingWord + 1)) + 1;
    wordIDFs[word] = idf;
  });
...
// to use
const thisdocument = extractTextContent();
const corpus = [thisdocument];
const topKeywords = calculateTFIDF(thisdocument, corpus);
